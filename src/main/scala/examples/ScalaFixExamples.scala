...
object PolyhedronExample extends ScalaFixExamples with App {
  System.load("/usr/local/lib/ppl/libppl_java.so")
  Parma_Polyhedra_Library.initialize_library()
  val cs = new Constraint_System
  val x0 = new Variable(0)  
  val constraint = new Constraint(new Linear_Expression_Variable(x0),
  Relation_Symbol.EQUAL,new Linear_Expression_Coefficient(new Coefficient(0)))
  cs.add(constraint)
    ...
    body = {
      (rho: Int => C_Polyhedron) => {
        ...
        case 2 => {
          ...
          pol.refine_with_constraint(new Constraint(new Linear_Expression_Variable(x0),
          Relation_Symbol.LESS_OR_EQUAL, new Linear_Expression_Coefficient(new Coefficient(10))))
          pol
        }
        case 3 => {
          val pol = new C_Polyhedron(rho(2))
          pol.affine_image(x0,new Linear_Expression_Sum(new Linear_Expression_Variable(x0), 
            new Linear_Expression_Coefficient(new Coefficient(1))), 
            new Coefficient(1)) // denominator
          pol    
        }
      }
    },
    inputUnknowns = Set(), // Set(0, 1, 2, 3),
    initial =  new C_Polyhedron(1,Degenerate_Element.EMPTY) ,  // InputAssignment.conditional(3, 10.0, 0.0),
...
object BoxExample extends ScalaFixExamples with App {
  System.load("/usr/local/lib/ppl/libppl_java.so")
  Parma_Polyhedra_Library.initialize_library()
  val cs = new Constraint_System
  val x0 = new Variable(0)  
  ...
  val simpleEqs: FiniteEquationSystem[Int, Double_Box] = FiniteEquationSystem(
    body = {
      (rho: Int => Double_Box) => {
        ...
        case 2 => {
          ...
          box.refine_with_constraint(new Constraint(new Linear_Expression_Variable(x0),
          Relation_Symbol.LESS_OR_EQUAL, new Linear_Expression_Coefficient(new Coefficient(10))))
          box
        }
        case 3 => {
          val box = new Double_Box(rho(2))
          box.affine_image(x0,new Linear_Expression_Sum(new Linear_Expression_Variable(x0), 
            new Linear_Expression_Coefficient(new Coefficient(1))), 
            new Coefficient(1)) // denominator
          box  
        }
    ...
    inputUnknowns = Set(), // Set(0, 1, 2, 3),
    initial =  new Double_Box(1,Degenerate_Element.EMPTY) ,  // InputAssignment.conditional(3, 10.0, 0.0),
    unknowns = Set(0, 1, 2, 3),
    infl = Relation(Map(0 -> Set(1), 1 -> Set(2), 2 -> Set(3), 3 -> Set(1))))
...
object JPPLBoxExample extends ScalaFixExamples with App {
  //usa i nuovi binding
  import it.unich.jppl._ ;
  val cs = new ConstraintSystem
  val x0 = 0 
  val constraint = new  it.unich.jppl.Constraint(
      new LinearExpression(0,1),Constraint.ConstraintType.EQUAL)
  cs.add(constraint)
  ...
  val simpleEqs: FiniteEquationSystem[Int, DoubleBox] = FiniteEquationSystem(
    body = {
      (rho: Int => DoubleBox) => {
        ...
        case 2 => new DoubleBox(rho(1)).refineWithConstraint(
          new Constraint(new LinearExpression(-10,1),Constraint.ConstraintType.LESS_OR_EQUAL))
        case 3 => new DoubleBox(rho(2)).affineImage(x0,new LinearExpression(1,1),new Coefficient(1))
        }
    },
    inputUnknowns = Set(), // Set(0, 1, 2, 3),
    initial =  new DoubleBox(1, Domain.DegenerateElement.EMPTY) ,  // InputAssignment.conditional(3, 10.0, 0.0),
    unknowns = Set(0, 1, 2, 3),
    infl = Relation(Map(0 -> Set(1), 1 -> Set(2), 2 -> Set(3), 3 -> Set(1))))
...
object GenericExample extends ScalaFixExamples with App {
  val cs = new Constraint_System
  val x0 = new Variable(0)  
  val constraint = new Constraint(new Linear_Expression_Variable(x0),
  Relation_Symbol.EQUAL,new Linear_Expression_Coefficient(new Coefficient(0)))
